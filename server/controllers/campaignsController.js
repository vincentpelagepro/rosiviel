...
import { apiRequest } from '../nodeAdwords/campaigns';
...
export const getData = (req, res) => {
  ...
  apiRequest(
    ...
    .then((result) => {
      ...
      // tableau de données MODIFIE pour les Campaigns
      const tableModified = tableSorted.map(lign => ({
        ...lign,
        Cost: lign.Cost / 1000000, // passer en dollars
        CPC: ((lign.Cost / 1000000) / lign.Clicks).toFixed(2), // rajout/calculer le cpc
      ...
      // renvoit des données pour les Campaigns
    ...
    .catch((error) => {
      // console.log('requète Api Campaigns: echec');
    });
};
