...
class ChartComponent extends React.Component {
  ...
  static propTypes = {
    ...
    type(props, propName, componentName) {
      if (!Chart.controllers[props[propName]]) {
        return new Error(`Invalid chart type \`${props[propName]}\` supplied to` +
          ` \`${componentName}\`.`);
      }
    },
  ...
  renderChart() {
    const {
      options, legend, type, redraw, plugins,
    } = this.props;
    const node = this.element;
  ...
  handleOnClick = (event) => {
    ...
    const activePoints = instance.getElementsAtEvent(event);
    ...
    //document.location.replace(label);
  }
